...
const IDLE_SESSION_TTL_MS = 2 * 60 * 1000;
...
export async function apiGet(page, token, path) {
  const result = await page.evaluate(async ({ url, token }) => {
    const response = await fetch(url, { headers: { Authorization: `Token token="${token}"` } });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    return response.json();
  }, { url: `${API_BASE}${path}`, token });
  if (String(result.tipo) !== '1') throw new Error(result.mensajes?.[0] ?? 'La sesión no fue aceptada por Logística.');
  return result;
}
