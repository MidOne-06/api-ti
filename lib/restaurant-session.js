...
// Restaurant expone los permisos efectivos de la sesión en su almacenamiento
// local. Esta lectura sucede dentro de la sesión autenticada vigente: no es un
// permiso estático del CRM ni una suposición basada en el rol local.
export async function restaurantPermissionTags(page) {
  return page.evaluate(() => {
    try {
      const current = JSON.parse(localStorage.getItem('ngStorage-currentUserLogistica') || '{}');
      return Array.isArray(current.permisoLogisticaList)
        ? current.permisoLogisticaList.map((permission) => String(permission?.tag ?? '')).filter(Boolean)
        : [];
    } catch {
      return [];
    }
